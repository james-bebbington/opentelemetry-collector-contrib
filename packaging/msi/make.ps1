...
function New-MSI(
    ...
    [string]$Config="./examples/tracing/otel-collector-config.yaml"
) {
    candle -arch x64 -dVersion="$Version" -dConfig="$Config" packaging/msi/opentelemetry-collector-contrib.wxs
    light opentelemetry-collector-contrib.wixobj
    Move-Item -Force opentelemetry-collector-contrib.msi bin/opentelemetry-collector-contrib.msi
}
...
function Confirm-MSI {
    ...
    Start-Process -Wait msiexec "/i `"$pwd\bin\opentelemetry-collector-contrib.msi`" /qn"
    ...
    Start-Process -Wait msiexec "/x `"$pwd\bin\opentelemetry-collector-contrib.msi`" /qn"
    sc.exe query state=all | findstr "otelcontribcol" | Out-Null
